...
| Prop | Type | Default | Description |
|---|---|---|---|
| `allowClean` | `boolean` | `false` | Show a clear-all button next to the chips |
| `allowNavigate` | `boolean` | `false` | Clicking a chip navigates the calendar to that date |
| `animated` | `boolean` | `false` | Animate chips appearing and disappearing |
...
```ts
type CalendarValue<M extends CalendarMode> =
