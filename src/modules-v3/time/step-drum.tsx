...
/**
 * v3 drum-picker primitive. The physics live in the pure `useTrack` /
 * `useItemSize` hooks (no v2 context coupling); this shell maps a numeric
 * value/index onto the spinning wheel and reports index changes back. Time,
 * months and years wheels all compose this with their own value mapping.
 */
...
export interface StepDrumProps {
  value: number;
  max: number;
  minValue?: number;
  maxValue?: number;
    ...
    >
      ...
      {OFFSETS.map((o) => {
        ...
        return (
          ...
          >
            {/* Non-breaking space: a plain " " collapses to a zero-height line
                box, so out-of-range cells in a finite (bound) wheel would
                shrink and the drum height would jump while scrolling. */}
