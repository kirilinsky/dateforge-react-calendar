...
describe("useConfig — prop overrides", () => {
  it("locale prop propagates", () => {
    const { result } = renderHook(() => useConfig(), { wrapper: wrap({ locale: "ru" }) });
    expect(result.current.locale).toBe("ru");
  });

  it("hour12=true propagates", () => {
    const { result } = renderHook(() => useConfig(), { wrapper: wrap({ hour12: true }) });
    expect(result.current.hour12).toBe(true);
  });

  it("gradient=true propagates", () => {
    const { result } = renderHook(() => useConfig(), { wrapper: wrap({ gradient: true }) });
    expect(result.current.gradient).toBe(true);
  });

  it("readOnly=true propagates", () => {
    const { result } = renderHook(() => useConfig(), { wrapper: wrap({ readOnly: true }) });
    expect(result.current.readOnly).toBe(true);
  });

  it("mode=range → range=true", () => {
    const { result } = renderHook(() => useConfig(), { wrapper: wrap({ mode: "range" }) });
    expect(result.current.range).toBe(true);
  });

  it("mode=multiple → multiselect truthy", () => {
    const { result } = renderHook(() => useConfig(), { wrapper: wrap({ mode: "multiple" }) });
